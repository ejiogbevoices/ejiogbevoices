...
CREATE POLICY "public_recordings_readable" ON recordings
  FOR SELECT USING (visibility = 'public');

CREATE POLICY "user_recordings_readable" ON recordings
  FOR SELECT USING (
    visibility = 'members' AND 
    auth.uid() IN (SELECT id FROM users WHERE role IN ('admin', 'editor', 'member'))
  );

CREATE POLICY "institution_recordings_readable" ON recordings
  FOR SELECT USING (
    visibility = 'institution' AND 
    auth.uid() IN (SELECT id FROM users WHERE org_id IS NOT NULL)
  );

CREATE POLICY "private_recordings_readable" ON recordings
  FOR SELECT USING (created_by = auth.uid());

CREATE POLICY "user_can_create_recordings" ON recordings
  FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "user_can_update_own_recordings" ON recordings
  FOR UPDATE USING (created_by = auth.uid());

CREATE POLICY "public_playlists_readable" ON playlists
  FOR SELECT USING (visibility = 'public');

CREATE POLICY "user_playlists_readable" ON playlists
  FOR SELECT USING (owner = auth.uid());

CREATE POLICY "user_can_create_playlists" ON playlists
  FOR INSERT WITH CHECK (auth.uid() = owner);

CREATE POLICY "user_can_update_own_playlists" ON playlists
  FOR UPDATE USING (owner = auth.uid());

CREATE POLICY "users_readable_self" ON users
  FOR SELECT USING (id = auth.uid());

CREATE POLICY "users_insertable_self" ON users
  FOR INSERT WITH CHECK (id = auth.uid());

CREATE POLICY "users_updatable_self" ON users
  FOR UPDATE USING (id = auth.uid());

CREATE POLICY "clips_readable" ON clips
  FOR SELECT USING (TRUE);

CREATE POLICY "user_can_create_clips" ON clips
  FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "user_can_update_own_clips" ON clips
  FOR UPDATE USING (created_by = auth.uid());

CREATE POLICY "events_readable_by_admins" ON events
  FOR SELECT USING (
    auth.uid() IN (SELECT id FROM users WHERE role = 'admin')
  );

CREATE POLICY "jobs_readable_by_admins" ON jobs
  FOR SELECT USING (
    auth.uid() IN (SELECT id FROM users WHERE role = 'admin')
  );
